...
{
    public partial class CompressD : Form
    {
        ...
        public CompressD(string folder, string output,string type)
        {
            InitializeComponent();
          
            cmbFormato.SelectedIndex = 2;
            cmbNivel.SelectedIndex = 5;
        ...
        private void directoryByType(string type)
        {
            if(type == "/com7z")
            {
                Text = "Comprimir como 7Zip | SharpCompress " + Application.ProductVersion.ToString();
                cmbFormato.SelectedIndex = 0; }
            else if (type == "/comTar")
            { Text = "Comprimir como Tar | SharpCompress " + Application.ProductVersion.ToString();
                cmbFormato.SelectedIndex = 1; }
            else if(type == "/comZip")
            {
                Text = "Comprimir como Zip | SharpCompress " + Application.ProductVersion.ToString();
                cmbFormato.SelectedIndex = 2; }
            else if (type == "/comGz")
            {
                Text = "Comprimir como Gz | SharpCompress " + Application.ProductVersion.ToString();
                cmbFormato.SelectedIndex = 3; }
            else
                Text = "Comprimir | SharpCompress " + Application.ProductVersion.ToString();
        }

        internal void ComprimirArchivosDesdeArgumentos(string[] args)
        {
            ...
            foreach (var arg in args)
            {
                ...
                {
                    string nombreArchivo = Path.GetFileName(arg);
                    archivos[nombreArchivo] = arg; // Agrega al diccionario
                }
            }
        ...
        private async void btnComprimir_Click(object sender, EventArgs e)
        {
            ...
            //TIPO DE METODO DE COMPRESION DEPENDIENDO DE LO SELECCIONADO EN EL CMBFORMATP
            if(cmbFormato.SelectedIndex == 0)
                tmp.CompressionMethod = com.GetCompressMethod7z(this);
            if (cmbFormato.SelectedIndex == 0)
                tmp.CompressionMethod = com.GetCompressMethodTar(this);
            if (cmbFormato.SelectedIndex == 0)
                tmp.CompressionMethod = com.GetCompressMethodZip(this);
